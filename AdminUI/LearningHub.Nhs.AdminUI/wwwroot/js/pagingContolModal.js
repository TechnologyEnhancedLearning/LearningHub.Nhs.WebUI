var AdminUIModal = AdminUIModal || {};

AdminUIModal.PagingRequest = { Page: 1, SortColumn: '', SortDirection: '', Filter: [], UseAjax: false };

AdminUIModal.pagingSetupAjax = function (sortColumn, sortDirection, filter, presetFilter, ajaxUrl, resultsElementId) {
    if (!presetFilter) { presetFilter = []; }
    AdminUIModal.PagingRequest.PresetFilter = presetFilter;

    AdminUIModal.PagingRequest.UseAjax = true;
    AdminUIModal.PagingRequest.AjaxUrl = ajaxUrl;
    AdminUIModal.PagingRequest.ResultsElementId = resultsElementId;
    AdminUIModal.pagingSetup(sortColumn, sortDirection, filter);
}

AdminUIModal.url = window.location.pathname;

AdminUIModal.pagingSetup = function (sortColumn, sortDirection, filter, $table, url) {
    if (url) {
        AdminUIModal.url = url;
    }
    AdminUIModal.PagingRequest.SortColumn = sortColumn;
    AdminUIModal.PagingRequest.SortDirection = sortDirection;

    if (!filter) { filter = []; }
    AdminUIModal.PagingRequest.Filter = filter;

    if (!$table) {
        $table = $('table.lh-datatable-modal');
    }
    AdminUIModal.initialiseTable($table);
};

AdminUIModal.initialiseTable = function ($table) {
    // Add Click events to column headers
    $table.find('th.orderable').attr('onClick', 'AdminUIModal.changeTableOrder(this);');

    // Add asc & desc class
    $table.find('th[data-column="' + AdminUIModal.PagingRequest.SortColumn + '"].orderable').addClass(AdminUIModal.PagingRequest.SortDirection === 'D' ? 'desc' : 'asc');
    ...
    if ($table.hasClass('filtered')) {
        ...
        for (var i = 0; i < columns.length; i++) {
            ...
            var filterColumn = AdminUIModal.PagingRequest.Filter.filter(function (x) { return x.Column === columnName; })[0];
            var filterValue = '';
            ...
            if (columnName) {
                ...
                $(row).append('<td class="action-column-modal"></td>');
            }
        ...
        $table.find('tr.filter-row td button').on('click', function () { AdminUIModal.filterTable($table); });
        $table.find('tr.filter-row td input').keypress(function (e) {
            if (e.which === 13) {
                AdminUIModal.filterTable($table);
                return false;
            }
        });
...
AdminUIModal.requestPage = function (page) {
    AdminUIModal.PagingRequest.Page = page;
    AdminUIModal.retrievePage();
};

AdminUIModal.changeTableOrder = function (headerLink) {
    var sortDirection = 'A';
    ...
    if (AdminUIModal.PagingRequest.UseAjax) {
        AdminUIModal.PagingRequest.Page = 1;
    }
    AdminUIModal.retrievePage();
};

AdminUIModal.filterTable = function ($table) {
    AdminUIModal.PagingRequest.Filter = [];
    $table.find('tbody tr.filter-row td input').each(function (index) {
        var filterColumn = $(this).attr('data-column');
        var filterValue = $(this).val();
        var dataType = $(this).attr('type');
        if ($(this).val() !== '') {
            AdminUIModal.PagingRequest.Filter.push({ Column: filterColumn, Value: filterValue, Type: dataType });
        }
    });
    if (AdminUIModal.PagingRequest.UseAjax) {
        AdminUIModal.PagingRequest.Page = 1;
...
AdminUIModal.clearFilters = function () {
    AdminUIModal.PagingRequest.Page = 1;
    AdminUIModal.PagingRequest.Filter = [];
    AdminUIModal.retrievePage();
    return false;
...
AdminUIModal.retrievePage = function () {
    if (AdminUIModal.PagingRequest.UseAjax) {
        var pagingRequestString = JSON.stringify(AdminUIModal.PagingRequest);
        var url = window.location.origin + '/' + AdminUIModal.PagingRequest.AjaxUrl;
        $.ajax({
            cache: false,
            type: "POST",
            url: url,
            ...
            success: function (data) {
                $(AdminUIModal.PagingRequest.ResultsElementId).html(data);
                $table = $(AdminUIModal.PagingRequest.ResultsElementId + ' table.lh-datatable-modal');
            ...
            error: function () {
                $(AdminUIModal.PagingRequest.ResultsElementId).html('<div class="alert alert-danger">Error loading page</div>');
            }
        });
    }
};
...
(function () {
    function AdminDatatable(config, request) {
        ...
        self.$table.find('th.orderable-modal').on('click', function () { self.changeTableOrder(this) });
