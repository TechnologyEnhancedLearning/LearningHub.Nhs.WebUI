...
using LearningHub.Nhs.WebUI.BlazorClient.Experiment;
...
try
{
    ...
    // Bff uses auth for testing with disabled auth on open api overide the service - also remember lh-api and open api have different models the need switching on too
    builder.Services.AddScoped<ILearningHubHttpClient, LearningHubWasmHttpClientExperiment>();
   ...
   // builder.Services.AddScoped(sp => new HttpClient { BaseAddress = new Uri(builder.HostEnvironment.BaseAddress) });
    ...
    //builder.Services.AddScoped<ISearchExperimentService, SearchExperimentServiceOpenApi>(); //work but lets use lh one
    builder.Services.AddScoped<LearningHub.Nhs.WebUI.Shared.Interfaces.ISearchService, LearningHub.Nhs.WebUI.Shared.Services.SearchService>();

    // Register IOptions<Settings> - requires manual binding in WASM
...
// --> qqqq MONDAY Important: In Blazor WASM, IOptions<T> works only if the configuration is available at runtime.That means you must ensure that your appsettings.json or other configuration is correctly loaded into builder.Configuration.
    ...
    builder.Services.AddScoped<ISearchExperimentService>(provider => provider.GetRequiredService<ISearchService>());
