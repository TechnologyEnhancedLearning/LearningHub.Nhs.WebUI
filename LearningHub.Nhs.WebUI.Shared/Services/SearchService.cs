namespace LearningHub.Nhs.WebUI.Shared.Services
{
    ...
    /// </summary>
    public class SearchService : LearningHub.Nhs.WebUI.Shared.Services.BaseService<LearningHub.Nhs.WebUI.Shared.Services.SearchService>, LearningHub.Nhs.WebUI.Shared.Interfaces.ISearchService/*, ISearchExperimentService*/
    {
        //private readonly Settings settings;
        private readonly IFindwiseSettings findwiseSettings;
        ...
        /// 
        //qqqq need to seperate settings
        public SearchService(ILearningHubHttpClient learningHubHttpClient, IProviderService providerService, ILogger<SearchService> logger, /*IOptions<LearningHub.Nhs.WebUI.Shared.Configuration.Settings> settings*/ IOptions<FindwiseSettings> findwiseSettings, IOptions<PublicSettings> publicSettings)
        //ORIGINAL public SearchService(ILearningHubHttpClient learningHubHttpClient, IProviderService providerService, ILogger<SearchService> logger, IOptions<LearningHub.Nhs.WebUI.Shared.Configuration.Settings> settings)
       ...
       // public SearchService(ILearningHubHttpClient learningHubHttpClient, IProviderService providerService, ILogger<SearchService> logger, IOptions<Settings> settings)
        ...
        {
            return (await GetAutoSuggestionList(term)).ConceptDocument.ConceptDocumentList
                .Select(x => x.Title)
                .Where(x => !string.IsNullOrWhiteSpace(x)) ?? Enumerable.Empty<string>();

        }

        public async Task<IEnumerable<string>> SearchAsync(string searchTerm, CancellationToken cancellationToken = default)
        {

            return (await GetAutoSuggestionList(searchTerm))?.ConceptDocument?.ConceptDocumentList
        ...
        public async Task<SearchResultViewModel> PerformSearch(IPrincipal user, SearchRequestViewModel searchRequest)
        {
            ...
            var resourceSearchPageSize = findwiseSettings.ResourceSearchPageSize;
            var catalogueSearchPageSize = findwiseSettings.CatalogueSearchPageSize;
        ...
        public async Task<int> SubmitFeedbackAsync(SearchFeedBackModel model)
        {
            try
            {
                ...
                var client = await this.LearningHubHttpClient.GetClientAsync();
                var request = publicSettings.LearningHubApiUrl + "Search/SubmitFeedback";
        ...
        {
            // qqqq having to get around the client fo sake of the demo so cant address the client itself really until it can hit the api in a later release
            //var client = await this.LearningHubHttpClient.GetClientAsync();
            var client = new HttpClient()
            {
                BaseAddress = new Uri("https://lh-openapi.dev.local/")
            };
            ...
            {
                ConceptDocument = new LearningHub.Nhs.Models.Search.AutoSuggestionConcept()
                {
                    ConceptDocumentList = new List<AutoSuggestionConceptDocument>()
                }
            };
            if (response.IsSuccessStatusCode)
            {
                // this doesnt work because json attributes are named wrong so just get what we care about
                var result = response.Content.ReadAsStringAsync().Result;
                ...
                Logger.LogInformation($"GetAutoSuggestionList: {result}");
                var modelsAreBrokenSoUseOwn = JsonConvert.DeserializeObject<OpenAPISearchResponse>(result);
                //var modelsAreBrokenSoUseOwn = JsonConvert.DeserializeObject<LHWebSearchResponse>(result);
                Logger.LogInformation($"modelsAreBrokenSoUseOwn: {modelsAreBrokenSoUseOwn}");
                modelsAreBrokenSoUseOwn.conceptDocument.conceptDocumentList.ForEach(x => 
                {
                    viewModel.ConceptDocument.ConceptDocumentList.Add(
                        new AutoSuggestionConceptDocument()
                        {
                            Title = x.title,
                            Click = new LearningHub.Nhs.Models.Search.SearchClick.AutoSuggestionClickModel()
                            {
                                Payload = new LearningHub.Nhs.Models.Search.SearchClick.AutoSuggestionClickPayloadModel
                                {
                                    ClickTargetUrl = x.click.payload.clickTargetUrl
                                }
                            }
                        }
                     );
                ...
                viewModel.ConceptDocument.ConceptDocumentList.Add(
                        new AutoSuggestionConceptDocument()
                        {
                            Title = "FYI LH Service",
                            Click = new LearningHub.Nhs.Models.Search.SearchClick.AutoSuggestionClickModel()
                            {
                                Payload = new LearningHub.Nhs.Models.Search.SearchClick.AutoSuggestionClickPayloadModel
                                {
                                    ClickTargetUrl = "FYI LH Service"
                                }
                            }
                        });

                //viewModel.ConceptDocument.ConceptDocumentList = modelsAreBrokenSoUseOwn.
                //    conceptDocument.ToList();
                //    .Select(x =>x.conceptDocument).ToList();
        ...
        private class OpenAPISearchResponse
        {
            //public LearningHub.Nhs.Models.Search.AutoSuggestionConceptDocument conceptDocument { get; set; }
             public OpenAPIConceptDocument conceptDocument { get; set; }
            // You could also add resourceDocument or catalogueDocument if needed
        }

        private class OpenAPIConceptDocument
        {
            public List<OpenAPIConceptItem> conceptDocumentList { get; set; }
        }

        private class OpenAPIConceptItem
        {
            public string id { get; set; }
            public string title { get; set; }
            public string concept { get; set; }
            public OpenAPIClick click { get; set; }
        }

        private class OpenAPIClick
        {
            public OpenAPIPayload payload { get; set; }
            public string p { get; set; }
        }

        private class OpenAPIPayload
        {
            public string clickTargetUrl { get; set; }
        }
        private class LHWebSearchResponse
        {
            [JsonProperty("concepts_documents")]
            public LHWebConceptDocument conceptDocument { get; set; }
        }

        private class LHWebConceptDocument
        {
            [JsonProperty("documents")]
            public List<LHWebConceptItem> conceptDocumentList { get; set; }
        }

        private class LHWebConceptItem
        {
            [JsonProperty("_id")]
            public string id { get; set; }
            ...
            [JsonProperty("_click")]
            public LHWebClick click { get; set; }
        }

        private class LHWebClick
        {
            [JsonProperty("payload")]
            public LHWebPayload payload { get; set; }
        }

        private class LHWebPayload
        {
            [JsonProperty("clickTargetUrl")]
            public string clickTargetUrl { get; set; }
        }
        /// <summary>
