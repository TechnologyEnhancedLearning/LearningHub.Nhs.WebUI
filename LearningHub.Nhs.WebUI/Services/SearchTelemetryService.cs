namespace LearningHub.Nhs.WebUI.Services
{
    ...
    public class SearchTelemetryService : ISearchTelemetryService
    {
        ...
        /// <param name="latencyMs">The search execution latency in milliseconds.</param>
        /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
        public Task RecordApiSearchExecutedAsync(SearchViewModel searchViewModel, string queryText, Guid groupId, long latencyMs)
        {
            ...
            try
            {
                // Extract result count from SearchViewModel
                // SearchViewModel contains DocumentModel which has the results
                int resultCount = searchViewModel.DocumentModel?.Count ?? 0;

                // Use SearchId as CorrelationId if available, otherwise generate one
                var correlationId = searchViewModel.SearchId > 0
                    ? searchViewModel.SearchId.ToString()
                    : Guid.NewGuid().ToString();
                ...
                var properties = new Dictionary<string, string>
                {
                    ...
                    { "QueryMode", "standard" }, // Default mode; could be enhanced if available in SearchViewModel
                    { "UseSemanticReranker", "false" }, // Default; could be enhanced if available in SearchViewModel
                    { "ResultType", "resource" }, // Default; could be enhanced if available in SearchViewModel
                };
            ...
            {
                // Log the exception to help with debugging, but don't let telemetry errors impact search functionality
                this.logger.LogError(ex, "Failed to record SearchExecutedTelemetry for query: {QueryText}", queryText);
            }
