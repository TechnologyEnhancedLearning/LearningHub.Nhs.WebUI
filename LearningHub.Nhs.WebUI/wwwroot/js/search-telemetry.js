...
LHGlobal.searchFacetTelemetry = new function () {
    var endpoint = '/api/Search/RecordFacetAppliedTelemetry';

    var recordFacetTelemetry = function (correlationId, sessionId, queryText, facetField, facetValue, facetAction) {
        var body = JSON.stringify({
            ...
            facetAction: facetAction || 'applied'
        });

        console.log('Recording facet telemetry:', {
            correlationId: correlationId,
            sessionId: sessionId,
            queryText: queryText,
            facetField: facetField,
    ...
    var getSearchMetadata = function () {
        // Get the form containing search filters
        var form = document.querySelector('.search-filters form');
        var searchIdInput = form ? form.querySelector('input[name="searchId"]') : null;
        var groupIdInput = form ? form.querySelector('input[name="groupId"]') : null;
        
        // Get search term from URL or from wherever it's stored
        var searchTerm = new URLSearchParams(window.location.search).get('term') || '';
        
        return {
            searchId: searchIdInput ? searchIdInput.value : '',
            groupId: groupIdInput ? groupIdInput.value : '',
            searchTerm: searchTerm
        };
    };

    // Note: Facet telemetry is now recorded server-side in SearchController.IndexPost
    // when the Apply button is submitted, so we don't track individual checkbox changes here.
    // The recordFacetTelemetry function is kept available above if needed for future use.
};
