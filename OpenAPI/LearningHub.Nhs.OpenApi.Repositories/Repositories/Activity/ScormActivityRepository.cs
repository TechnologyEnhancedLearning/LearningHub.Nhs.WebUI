...
namespace LearningHub.Nhs.OpenApi.Repositories.Repositories.Activity
{
    ...
    public class ScormActivityRepository : GenericRepository<ScormActivity>, IScormActivityRepository
    {
        ...
        {
            return DbContext.ScormActivity.AsNoTracking().Where(s => s.Id == id)
        ...
        public int Create(int userId, int resourceReferenceId)
        {
            ...
            DbContext.Database.ExecuteSqlRaw("[activity].[ScormActivityCreate] @p0, @p1, @p2, @p3 output", param0, param1, param2, param3);
        ...
        public int Complete(int userId, int scormActivityId)
        {
            ...
            DbContext.Database.ExecuteSqlRaw("[activity].[ScormActivityComplete] @p0, @p1, @p2, @p3 output", param0, param1, param2, param3);
        ...
        public void Resolve(int scormActivityId)
        {
            ...
            var param1 = new SqlParameter("@p1", SqlDbType.Int) { Value = TimezoneOffsetManager.UserTimezoneOffset ?? (object)DBNull.Value };

            DbContext.Database.ExecuteSqlRaw("[activity].[ScormActivityResolve] @p0, @p1", param0, param1);
        }
        ...
        public ScormActivitySummaryDto GetScormActivitySummary(int userId, int resourceReferenceId)
        {
            ...
            DbContext.Database.ExecuteSqlRaw("[activity].[ScormActivityGetSummary] @p0, @p1, @p2 output, @p3 output", param0, param1, param2, param3);
        ...
        public override async Task UpdateAsync(int userId, ScormActivity updatedScormActivity)
        {
            try
            {
                await UpdateScormActivityAsync(userId, updatedScormActivity);
            }
        ...
        {
            var incompleteScormActivity = GetByIdAsync(incompleteScormActivityId).Result;

            var scormActivity = DbContext.ScormActivity
        ...
        private async Task UpdateScormActivityAsync(int userId, ScormActivity updatedScormActivity)
        {
            ...
            DbContext.Entry(existingScormActivity).CurrentValues.SetValues(updatedScormActivity);
            ...
            foreach (var interaction in updatedScormActivity.ScormActivityInteraction)
            {
                ...
                if (existingInteraction != null)
                {
                    ...
                    DbContext.Entry(existingInteraction).CurrentValues.SetValues(interaction);
                ...
                {
                    SetAuditFieldsForCreate(userId, interaction);
                    foreach (var correctResponse in interaction.ScormActivityInteractionCorrectResponse)
                    {
                        SetAuditFieldsForCreate(userId, correctResponse);
                    }

                    foreach (var objective in interaction.ScormActivityInteractionObjective)
                    {
                        SetAuditFieldsForCreate(userId, objective);
                    }
            ...
            foreach (var objective in updatedScormActivity.ScormActivityObjective)
            {
                ...
                if (existingObjective != null)
                {
                    ...
                    DbContext.Entry(existingObjective).CurrentValues.SetValues(objective);
