namespace LearningHub.Nhs.OpenApi.Repositories.Repositories.Content
{
    ...
    public class PageSectionDetailRepository : GenericRepository<PageSectionDetail>, IPageSectionDetailRepository
    {
        ...
        {
            return await DbContext.PageSectionDetail
        ...
        public async new Task<int> CreateAsync(int userId, PageSectionDetail pageSectionDetail)
        {
            ...
            {
                SetAuditFieldsForCreate(userId, pageSectionDetail.ImageAsset);
            }
        ...
        public async new Task UpdateAsync(int userId, PageSectionDetail pageSectionDetail)
        {
            if (pageSectionDetail.ImageAsset != null)
            {
                SetAuditFieldsForUpdate(userId, pageSectionDetail.ImageAsset);
            }
        ...
        public async Task<PageSectionDetail> CloneSectionDetailAsync(int pageSectionDetailId, SectionTemplateType sectionTemplateType, int currentUserId)
        {
            ...
            {
                pageSectionDetail = await DbContext.PageSectionDetail.Where(ps => ps.Id == pageSectionDetailId).AsNoTracking().Include(psd => psd.VideoAsset).AsNoTracking().SingleAsync();
            ...
            {
                pageSectionDetail = await DbContext.PageSectionDetail.Where(ps => ps.Id == pageSectionDetailId).AsNoTracking().Include(psd => psd.ImageAsset).AsNoTracking().SingleAsync();
            ...
            pageSectionDetail.Id = await CreateAsync(currentUserId, pageSectionDetail);

            if (sectionTemplateType == SectionTemplateType.Video)
            {
                pageSectionDetail = await GetPageSectionDetailVideoAssetByIdAsync(pageSectionDetail.Id);
            }
            else
            {
                pageSectionDetail = await GetPageSectionDetailImageAssetByIdAsync(pageSectionDetail.Id);
            }
