namespace LearningHub.Nhs.OpenApi.Repositories.Repositories.Hierarchy
{
    ...
    public class NodeRepository : GenericRepository<Node>, INodeRepository
    {
        ...
        {
            return await DbContext.Node.AsNoTracking().FirstOrDefaultAsync(r => r.Id == id && !r.Deleted);
        }
        ...
        public async Task<NodeViewModel> GetNodeDetails(int nodeId)
        {
            ...
            var retVal = await DbContext.NodeViewModel.FromSqlRaw("hierarchy.GetNodeDetails @p0", param0).AsNoTracking().ToListAsync();
        ...
        public async Task<List<NodeContentBrowseViewModel>> GetNodeContentsForCatalogueBrowse(int nodeId, bool includeEmptyFolder)
        {
            ...
            {
                var retVal = await DbContext.NodeContentBrowseViewModel.FromSqlRaw("hierarchy.GetNodeContentsForCatalogueBrowse_withEmptyFolders @p0", param0).AsNoTracking().ToListAsync();
                return retVal;
            }
            else
            {
                var retVal = await DbContext.NodeContentBrowseViewModel.FromSqlRaw("hierarchy.GetNodeContentsForCatalogueBrowse @p0", param0).AsNoTracking().ToListAsync();
                return retVal;
            }
        }
        ...
        public async Task<List<NodeContentEditorViewModel>> GetNodeContentsForCatalogueEditor(int nodeId)
        {
            ...
            var retVal = await DbContext.NodeContentEditorViewModel.FromSqlRaw("hierarchy.GetNodeContentsForCatalogueEditor @p0", param0).AsNoTracking().ToListAsync();
            return retVal;
        }
        ...
        public async Task<List<NodeContentAdminViewModel>> GetNodeContentsAdminAsync(int nodeId, bool readOnly)
        {
            try
            {
                ...
                if (readOnly)
                {
                    var retVal = await DbContext.NodeContentAdminViewModel.FromSqlRaw("hierarchy.GetNodeContentsAdminReadOnly @p0", param0).AsNoTracking().ToListAsync();
                    return retVal;
                }
                else
                {
                    var retVal = await DbContext.NodeContentAdminViewModel.FromSqlRaw("hierarchy.GetNodeContentsAdmin @p0", param0).AsNoTracking().ToListAsync();
                    return retVal;
                }
            }
            catch
