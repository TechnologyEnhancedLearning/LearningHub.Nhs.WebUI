namespace LearningHub.Nhs.OpenApi.Repositories.Repositories.Hierarchy
{
    ...
    public class PublicationRepository : GenericRepository<Publication>, IPublicationRepository
    {
        ...
        {
            return await DbContext.Publication.AsNoTracking().FirstOrDefaultAsync(r => r.Id == id && !r.Deleted);
        }
        ...
        {
            return DbContext.Publication.AsNoTracking().FirstOrDefault(r => r.Id == id && !r.Deleted);
        }
        ...
        {
            return await DbContext.Publication.AsNoTracking().FirstOrDefaultAsync(r => r.ResourceVersionId == resourceVersionId && !r.Deleted);
        }
        ...
        public async Task<List<CacheOperationViewModel>> GetCacheOperations(int publicationId)
        {
            ...
            var vm = await DbContext.CacheOperationViewModel.FromSqlRaw("hierarchy.GetCacheRefreshDetails @p0", param0).ToListAsync();
        ...
        {
            return await (from pub in DbContext.Publication.AsNoTracking()
                          join rv in DbContext.ResourceVersion on pub.ResourceVersionId equals rv.Id
                          join r in DbContext.Resource on rv.ResourceId equals r.Id
