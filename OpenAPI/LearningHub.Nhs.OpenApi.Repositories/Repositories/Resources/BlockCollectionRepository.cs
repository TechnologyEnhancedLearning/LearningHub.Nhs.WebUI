    ...
    public class BlockCollectionRepository : GenericRepository<BlockCollection>, IBlockCollectionRepository
    {
        ...
        public async Task<BlockCollection> GetBlockCollection(int? blockCollectionId)
        {
            ...
            await Task.WhenAll(blockCollection.Blocks
                ...
                .Select(block => FillInPartialQuestionBlock(block.QuestionBlock)));

            await Task.WhenAll(blockCollection.Blocks
                .Where(block => block.ImageCarouselBlock != null)
                .Select(block => FillInPartialImageCarouselBlock(block.ImageCarouselBlock)));
        ...
        public async Task<string> GetResourceBlockCollectionsFileAsync(int excludeResourceVersionId, ResourceTypeEnum resourceTypeEnum)
        {
            if (excludeResourceVersionId > 0)
            {
                ...
                await DbContext.Database.ExecuteSqlRawAsync("[resources].[BlockCollectionFileSearch] @excludeResourceVersionId, @resourceType, @filePath output", param0, param1, param2);
        ...
        {
            List<Block> questionBlocks = await DbContext.Block
                .AsNoTracking()
        ...
        {
            var questionTask = GetBlockCollection(questionBlock.QuestionBlockCollectionId);
            var feedbackTask = GetBlockCollection(questionBlock.FeedbackBlockCollectionId);
            var answersTask = Task.WhenAll(questionBlock.Answers
                .Select(async answer =>
                {
                    var blockCollection = await GetBlockCollection(answer.BlockCollectionId);
        ...
        {
            var imageTask = GetBlockCollection(imageCarouselBlock.ImageBlockCollectionId);
        ...
        {
            SetAuditFieldsForCreateOrDelete(userId, blockCollection, isCreate);
            foreach (var block in blockCollection.Blocks)
            {
                SetAuditFieldsForCreateOrDelete(userId, block, isCreate);
                SetAuditFieldsOnChildren(userId, block, isCreate);
            }
        }
        ...
        private void SetAuditFieldsOnChildren(int userId, Block block, bool isCreate)
        {
            ...
            {
                SetAuditFieldsForCreateOrDelete(userId, block.TextBlock, isCreate);
            }

            if (block.MediaBlock != null)
            {
                SetAuditFieldsForCreateOrDelete(userId, block.MediaBlock, isCreate);
                SetAuditFieldsOnChildren(userId, block.MediaBlock, isCreate);
            }

            if (block.WholeSlideImageBlock != null)
            {
                SetAuditFieldsForCreateOrDelete(userId, block.WholeSlideImageBlock, isCreate);
                SetAuditFieldsOnChildren(userId, block.WholeSlideImageBlock, isCreate);
            }

            if (block.QuestionBlock != null)
            {
                SetAuditFieldsForCreateOrDelete(userId, block.QuestionBlock, isCreate);
                SetAuditFieldsOnChildren(userId, block.QuestionBlock, isCreate);
            }

            if (block.ImageCarouselBlock != null)
            {
                SetAuditFieldsForCreateOrDelete(userId, block.ImageCarouselBlock, isCreate);
                SetAuditFieldsOnChildren(userId, block.ImageCarouselBlock, isCreate);
            }
        }
        ...
        private void SetAuditFieldsOnChildren(int userId, MediaBlock mediaBlock, bool isCreate)
        {
            ...
            {
                SetAuditFieldsForCreateOrDelete(userId, mediaBlock.Attachment, isCreate);
            }
            else if (mediaBlock.Image != null)
            {
                SetAuditFieldsForCreateOrDelete(userId, mediaBlock.Image, isCreate);
            }
            else if (mediaBlock.Video != null)
            {
                SetAuditFieldsForCreateOrDelete(userId, mediaBlock.Video, isCreate);
            }
        }
        ...
        private void SetAuditFieldsOnChildren(int userId, WholeSlideImageBlock wholeSlideImageBlock, bool isCreate)
        {
            ...
            {
                SetAuditFieldsForCreateOrDelete(userId, wholeSlideImageBlockItem, isCreate);
                SetAuditFieldsOnChildren(userId, wholeSlideImageBlockItem, isCreate);
            }
        }
        ...
        private void SetAuditFieldsOnChildren(int userId, WholeSlideImageBlockItem wholeSlideImageBlockItem, bool isCreate)
        {
            ...
            {
                SetAuditFieldsForCreateOrDelete(userId, wholeSlideImageBlockItem.WholeSlideImage, isCreate);
            }
        }

        private void SetAuditFieldsOnChildren(int userId, QuestionBlock questionBlock, bool isCreate)
        {
            SetAuditFieldsForCreateOrDelete(userId, questionBlock.QuestionBlockCollection, isCreate);
            SetAuditFieldsOnChildren(userId, questionBlock.QuestionBlockCollection, isCreate);

            SetAuditFieldsForCreateOrDelete(userId, questionBlock.FeedbackBlockCollection, isCreate);
            SetAuditFieldsOnChildren(userId, questionBlock.FeedbackBlockCollection, isCreate);

            foreach (QuestionAnswer answer in questionBlock.Answers)
            {
                SetAuditFieldsForCreateOrDelete(userId, answer, isCreate);
                if (answer.BlockCollection != null)
                {
                    SetAuditFieldsForCreateOrDelete(userId, answer.BlockCollection, isCreate);
                    SetAuditFieldsOnChildren(userId, answer.BlockCollection, isCreate);
                }
            }
        }
        ...
        private void SetAuditFieldsOnChildren(int userId, ImageCarouselBlock imageCarouselBlock, bool isCreate)
        {
            ...
            SetAuditFieldsForCreateOrDelete(userId, imageCarouselBlock.ImageBlockCollection, isCreate);
            SetAuditFieldsOnChildren(userId, imageCarouselBlock.ImageBlockCollection, isCreate);
        }
    }
}
