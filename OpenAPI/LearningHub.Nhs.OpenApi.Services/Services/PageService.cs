namespace LearningHub.Nhs.OpenApi.Services.Services
{
    ...
    public class PageService : IPageService
    {
        ...
        /// <param name="learningHubConfig">The learningHub Config.</param>
        ...
        {
            await pageRepository.DiscardAsync(pageId, currentUserId);
        }
        ...
        public async Task<PageViewModel> GetPageByIdAsync(int id, bool includeHidden = false, bool publishedOnly = false, bool preview = false)
        {
            ...
            var pageViewModel = new PageViewModel
            {
                ...
                PageSections = page.PageSections.Select(ps => new PageSectionViewModel
                {
                    ...
                    PageSectionDetail = mapper.Map<PageSectionDetailViewModel>(ps.PageSectionDetails.First()),
                })
        ...
        public async Task<PageResultViewModel> GetPagesAsync()
        {
            try
            {
                var pages = await pageRepository.GetPagesAsync();
        ...
        {
            var pageSection = await pageSectionRepository.GetByIdAsync(id);
            return mapper.Map<PageSectionViewModel>(pageSection);
        }
        ...
        {
            var detail = await pageSectionDetailRepository.GetPageSectionDetailImageAssetByIdAsync(id);

            return mapper.Map<PageSectionDetailViewModel>(detail);
        }
        ...
        public async Task<PageSectionDetailViewModel> GetEditablePageSectionDetailByPageSectionIdAsync(int pageSectionId, int currentUserId)
        {
            ...
            var pageSection = await pageSectionRepository.GetAll().Where(ps => ps.Id == pageSectionId)
            ...
            if (pageSection == null)
            {
                throw new Exception($"GetEditablePageSectionDetailByPageSectionIdAsync: PageSectionId {pageSectionId} not valid!");
            }

            if (!pageSection.PageSectionDetails.Any())
            {
                var liveDetail = await pageSectionDetailRepository.GetAll()
                                .Where(psd => psd.PageSectionId == pageSectionId && psd.PageSectionStatusId == (int)PageSectionStatus.Live)
                                .FirstOrDefaultAsync();

                detail = await pageSectionDetailRepository.CloneSectionDetailAsync(liveDetail.Id, (SectionTemplateType)pageSection.SectionTemplateTypeId, currentUserId);
            }
            else
            {
                if (pageSection.SectionTemplateTypeId == (int)SectionTemplateType.Video)
                {
                    detail = await pageSectionDetailRepository.GetPageSectionDetailVideoAssetByIdAsync(pageSection.PageSectionDetails.First().Id);
                }
                else
                {
                    detail = await pageSectionDetailRepository.GetPageSectionDetailImageAssetByIdAsync(pageSection.PageSectionDetails.First().Id);
                }
            }
        ...
        {
            await pageRepository.PublishAsync(pageId, currentUserId);
        }
        ...
        public async Task UpdatePageImageSectionDetailAsync(int pageId, PageImageSectionUpdateViewModel model, int currentUserId)
        {
            ...
            {
                var fileType = await fileTypeService.GetByFilename(model.ImageFileName);

                fileId = await fileRepository.CreateAsync(currentUserId, new File
                {
            ...
            var detail = await pageSectionDetailRepository.GetPageSectionDetailImageAssetByIdAsync(model.PageSectionDetailId.Value);
            ...
            await pageSectionDetailRepository.UpdateAsync(currentUserId, detail);
        }
        ...
        {
            var pageSection = await pageSectionRepository.GetByIdAsync(pageSectionId);
            if ((SectionTemplateType)pageSection.SectionTemplateTypeId == SectionTemplateType.Image)
            {
                await pageSectionRepository.CloneImageSectionAsync(pageSectionId, currentUserId);
            }
            else if ((SectionTemplateType)pageSection.SectionTemplateTypeId == SectionTemplateType.Video)
            {
                await pageSectionRepository.CloneVideoSectionAsync(pageSectionId, currentUserId);
            }
        }
        ...
        public async Task ChangeOrderAsync(UpdatePageSectionOrderModel requestViewModel, int currentUserId)
        {
            ...
            {
                await pageSectionRepository.ChangeOrderDownAsync(requestViewModel.PageId, requestViewModel.PageSectionId, currentUserId);
            }
            else
            {
                await pageSectionRepository.ChangeOrderUpAsync(requestViewModel.PageId, requestViewModel.PageSectionId, currentUserId);
            }
        }
        ...
        {
            await pageSectionRepository.HideAsync(pageSectionId, currentUserId);
        }
        ...
        {
            await pageSectionRepository.UnHideAsync(pageSectionId, currentUserId);
        }
        ...
        {
            await pageSectionRepository.DeleteAsync(pageSectionId, currentUserId);
        }
        ...
        public async Task<int> CreatePageSectionAsync(PageSectionViewModel requestViewModel, int currentUserId)
        {
            try
            {
                var pageSectionModel = mapper.Map<PageSection>(requestViewModel);
                var pageSectionDetail = mapper.Map<PageSectionDetail>(requestViewModel.PageSectionDetail);
                var pageSectionId = await pageSectionRepository.CreateWithPositionAsync(currentUserId, pageSectionModel);
                ...
                pageSectionDetail.Id = await pageSectionDetailRepository.CreateAsync(currentUserId, pageSectionDetail);
        ...
        public async Task<LearningHubValidationResult> SaveVideoAssetAsync(FileCreateRequestViewModel requestViewModel, int userId)
        {
            ...
            int fileId = await fileRepository.CreateAsync(userId, newFile);
            var videoAsset = await videoAssetRepository.GetByPageSectionDetailId(requestViewModel.PageSectionDetailId);
            ...
            await UpdateVideoAssetStateAsync(updateVideoAssetStateViewModel);
            ...
            if (videoAsset == null)
            {
                ...
                await videoAssetRepository.CreateAsync(userId, videoAsset);
            }
            else
            {
                ...
                await videoAssetRepository.UpdateAsync(userId, videoAsset);
            }
            ...
            var internalSystem = await internalSystemService.GetByIdAsync((int)InternalSystemType.ContentManagementQueue);
            var contentManagementQueue = internalSystem.IsOffline ? $"{learningHubConfig.ContentManagementQueueName}-temp" : learningHubConfig.ContentManagementQueueName;
            await queueCommunicatorService.SendAsync(contentManagementQueue, processVideoAssetQueueMessage);
        ...
        {
            var videoAsset = await videoAssetRepository.GetByPageSectionDetailId(requestViewModel.PageSectionDetailId);
            int fileId = await fileRepository.CreateAsync(
                currentUserId,
                new File()
                {
                    FileTypeId = requestViewModel.FileTypeId,
            ...
            await videoAssetRepository.UpdateAsync(currentUserId, videoAsset);
        ...
        {
            var pageSectionDetail = await pageSectionDetailRepository.GetPageSectionDetailImageAssetByIdAsync(model.Id);
            ...
            await pageSectionDetailRepository.UpdateAsync(currentUserId, pageSectionDetail);
        }
        ...
        {
            var pageSectionDetail = await pageSectionDetailRepository.GetPageSectionDetailImageAssetByIdAsync(videoAssetStateViewModel.PageSectionDetailId);
            pageSectionDetail.PageSectionStatusId = (int)videoAssetStateViewModel.PageSectionStatus;
            await pageSectionDetailRepository.UpdateAsync(videoAssetStateViewModel.UserId, pageSectionDetail);
        }
        ...
        {
            var detail = await pageSectionDetailRepository.GetPageSectionDetailVideoAssetByIdAsync(id);
        ...
        {
            var videoAsset = await videoAssetRepository.GetByPageSectionDetailId(viewModel.PageSectionDetailId);
            ...
            await videoAssetRepository.UpdateAsync(viewModel.UserId, videoAsset);
        }
        ...
        {
            var videoAsset = await videoAssetRepository.GetById(viewModel.Id);
