namespace LearningHub.Nhs.OpenApi.Services.Services
{
    ...
    public class RatingService : IRatingService
    {
        ...
        {
            var ratingSummary = await resourceVersionRatingSummaryRepository.GetByResourceVersionIdAsync(entityVersionId);
            RatingSummaryViewModel ratingSummaryViewModel;

            if (ratingSummary != null)
            {
                ratingSummaryViewModel = mapper.Map<RatingSummaryViewModel>(ratingSummary);
            }
            else
            {
            ...
            ratingSummaryViewModel.UserIsContributor = await IsTheUserTheContributor(userId, entityVersionId);
            ratingSummaryViewModel.UserCanRate = await HasUserCompletedActivity(userId, entityVersionId);

            var userRating = await resourceVersionRatingRepository.GetUsersPreviousRatingForSameMajorVersionAsync(entityVersionId, userId);
            if (userRating != null)
            {
        ...
        public async Task<RatingSummaryBasicViewModel> GetRatingSummaryBasic(int entityVersionId)
        {
            ...
            var retVal = await cachingService.GetAsync<RatingSummaryBasicViewModel>(cacheKey);
            ...
            else
            {
                try
                {
                    ...
                    var ratingSummaryBasicViewModel = mapper.Map<RatingSummaryBasicViewModel>(ratingSummary);
                    ...
                    await cachingService.SetAsync(cacheKey, ratingSummaryBasicViewModel);
        ...
        public async Task<LearningHubValidationResult> CreateRating(int userId, RatingViewModel ratingViewModel)
        {
            ...
            var retVal = await ValidateRating(userId, ratingViewModel);

            // Check user has not already rated the resource. Only applies to create, not update.
            if (await HasTheUserAlreadyRatedThisMajorVersion(userId, ratingViewModel.EntityVersionId))
            {
            ...
            await CreateNewRatingAndRecalcAverageAsync(userId, ratingViewModel);

            // Force cache refresh
            string cacheKey = $"{CacheKeys.RatingSummaryBasic}:{ratingViewModel.EntityVersionId}";
            await cachingService.RemoveAsync(cacheKey);

            await GetRatingSummaryBasic(ratingViewModel.EntityVersionId);
        ...
        public async Task<LearningHubValidationResult> UpdateRating(int userId, RatingViewModel ratingViewModel)
        {
            ...
            var oldRating = await resourceVersionRatingRepository.GetUsersPreviousRatingForSameMajorVersionAsync(ratingViewModel.EntityVersionId, userId);
            if (oldRating == null)
            {
            ...
            await resourceVersionRatingRepository.UpdateAsync(userId, oldRating);
        ...
        private async Task<LearningHubValidationResult> ValidateRating(int userId, RatingViewModel ratingViewModel)
        {
            ...
            if (!await IsTheCurrentVersion(ratingViewModel.EntityVersionId))
            {
            ...
            if (await IsTheUserTheContributor(userId, ratingViewModel.EntityVersionId))
            {
            ...
            if (!await HasUserCompletedActivity(userId, ratingViewModel.EntityVersionId))
            {
        ...
        private async Task CreateNewRatingAndRecalcAverageAsync(int userId, RatingViewModel ratingViewModel)
        {
            ...
            await resourceVersionRatingRepository.CreateAsync(userId, resourceVersionRating);

            // Recalculate the resource's star rating.
            var ratingSummary = await resourceVersionRatingSummaryRepository.GetByResourceVersionIdAsync(ratingViewModel.EntityVersionId);

            var allRatings = await resourceVersionRatingRepository.GetRatingCountsForResourceVersionAsync(ratingViewModel.EntityVersionId);
            ...
            await resourceVersionRatingSummaryRepository.UpdateAsync(userId, ratingSummary);
        ...
        {
            var publication = await publicationRepository.GetByResourceVersionIdAsync(entityVersionId);
        ...
        {
            var rating = await resourceVersionRatingRepository.GetUsersPreviousRatingForSameMajorVersionAsync(entityVersionId, userId);
            return rating != null;
        }

        private async Task<bool> HasUserCompletedActivity(int userId, int entityVersionId)
        {
            var hasCompleted = await resourceVersionRepository.HasUserCompletedActivity(userId, entityVersionId);
            return hasCompleted;
        }
    }
}