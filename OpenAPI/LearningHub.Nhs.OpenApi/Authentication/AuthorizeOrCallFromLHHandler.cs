namespace LearningHub.NHS.OpenAPI.Authentication
{
    ...
    public class AuthorizeOrCallFromLHHandler : AuthorizationHandler<AuthorizeOrCallFromLHRequirement>
    {
        ...
        public AuthorizeOrCallFromLHHandler(IHttpContextAccessor contextAccessor, IOptions<LearningHubConfig> settings)
        {
                ...
                if (headers.ContainsKey("Client-Identity-Key"))
                {
                    ...
                    if (headers.TryGetValue("Client-Identity-Key", out clientKeyValues))
                    {
                        ...
                        callFromAuthService = clientKey
                                          == settings.Value.AuthClientIdentityKey.ToUpperInvariant();

                        callFromLearningHubClient = clientKey
                                                == settings.Value.LHClientIdentityKey.ToUpperInvariant();

                        callFromLHContentServerClient = clientKey
                                                == settings.Value.ContentServerClientIdentityKey.ToUpperInvariant();

                        callFromReportApiClient = clientKey
                                               == settings.Value.ReportApiClientIdentityKey.ToUpperInvariant();
                    }
                }
            }
