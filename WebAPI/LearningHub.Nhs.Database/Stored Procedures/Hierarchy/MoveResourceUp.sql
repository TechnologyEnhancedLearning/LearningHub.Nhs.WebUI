...
-- Author       RS
-- Created      19-10-2021
-- Purpose      Move a resource up in the display order of a folder in real-time (Content Structure iteration 1)
--
...
-------------------------------------------------------------------------------
CREATE PROCEDURE [hierarchy].[MoveResourceUp] 
(
	@NodeId int,
	@ResourceId int,
	@UserId int,
...
BEGIN
	...
	BEGIN TRY
		...
		BEGIN
			RAISERROR ('Unable to move the resource up because the catalogue is currently locked for editing',
					16,	-- Severity.  
					1	-- State.  
					); 
		END
		...
		-- Check that the resource isn't already top of the list (excluding drafts).
		IF @CurrentDisplayOrder = 1
		BEGIN
			RAISERROR ('Cannot move the resource up in the folder display order',
					16,	-- Severity.  
					1	-- State.  
					); 
		END
