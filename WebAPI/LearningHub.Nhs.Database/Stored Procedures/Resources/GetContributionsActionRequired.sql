...
-------------------------------------------------------------------------------
CREATE PROCEDURE [resources].[GetContributionsActionRequired]
	@CatalogueNodeId		INT,
	@UserId					INT,
	@Offset					INT,
	@Take					INT,
...
BEGIN
	...
	SELECT	DISTINCT 
			...
			ISNULL(rvd.Id, 0) AS DraftResourceVersionId, 
			...
			CASE WHEN (SELECT COUNT(*) FROM resources.ResourceVersionValidationResult  rvvr WHERE rvvr.ResourceVersionId = rv.id AND rv.VersionStatusId = 6) > 0 THEN CAST(1 AS BIT) ELSE CAST(0 AS BIT) END AS HasValidationErrors,
			rv.AmendDate
	FROM	
	...
	WHERE	
		...
		AND (  rv.VersionStatusId = 6 /* FailedToPublish */
			OR (rv.VersionStatusId in (2, 3) AND r.CurrentResourceVersionId = rv.Id AND rvf.id IS NOT NULL) /* Published or Unpublished and Flagged */ 
			)
