...
namespace LearningHub.Nhs.Repository.Activity
{
    ...
    public class ResourceActivityRepository : GenericRepository<ResourceActivity>, IResourceActivityRepository
    {
        ...
        /// <param name="detailedMediaActivityRecordingStartDate">detailedMediaActivityRecordingStartDate.</param>
        /// <returns>ResourceActivity.</returns>
        public int GetTotalCount(int userId, Models.MyLearning.MyLearningRequestModel requestModel, DateTimeOffset detailedMediaActivityRecordingStartDate)
        {
            (DateTimeOffset? startDate, DateTimeOffset? endDate) = this.ApplyDatesFilter(requestModel);
            (string strResourceTypes, bool resourceTypeFlag) = this.ApplyResourceTypesfilters(requestModel);
            (string strActivityStatus, bool activityStatusEnumFlag) = this.ApplyActivityStatusFilter(requestModel);
            ...
            this.DbContext.Database.ExecuteSqlRaw("[activity].[GetUserLearningActivitiesCount] @userId, @searchText, @activityStatuses, @resourceTypes,@activityStartDate,@activityEndDate,@mediaActivityRecordingStartDate,@certificateEnabled,@totalcount output", param0, param1, param2, param3, param4, param5, param6, param7, param8);
        ...
        {
            var activities = this.GetByUserIdWithResourceVersionId(userId, resourceVersionId).Result.OrderBy(r => r.AmendDate).ToList();
            return activities;
        }
        ...
        {
            this.BindNodePathNestedData(result);
            this.BindResourceNestedData(result);
            this.BindResourceVersionData(result);
            this.BindScormActivityNestedData(result);
            this.BindAssessmentResourceActivityNestedData(result);
            this.BindMediaResourceActivityNestedData(result);
            this.BindResourceVersionBlockData(result);
            this.BindScormResourceVersionManifestNestedData(result);
        }
