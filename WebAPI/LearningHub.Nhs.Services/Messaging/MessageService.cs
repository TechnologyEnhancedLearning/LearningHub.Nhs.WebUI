// <copyright file="MessageService.cs" company="NHS England">
...
namespace LearningHub.Nhs.Services.Messaging
{
    ...
    public class MessageService : BaseService<Message>, IMessageService
    {
        ...
        {
            await this.messageRepository.CreateNotificationForUserAsync(userId, subject, body, recipientUserId, notificationStartDate, notificationEndDate, notificationPriority, notificationType);
        }
        ...
        {
            var messages = this.messageRepository.GetPendingMessages().ToList();
        ...
        public async Task<LearningHubValidationResult> MarkAsFailedAsync(int userId, List<int> messageFailedIds)
        {
            try
            {
                await this.messageRepository.MessageSendFailure(userId, messageFailedIds);
            }
            catch (Exception ex)
            {
        ...
        public async Task<LearningHubValidationResult> MarkAsSentAsync(int userId, List<int> messageSendIds)
        {
            try
            {
                await this.messageRepository.MessageSendSuccess(userId, messageSendIds);
            }
            catch (Exception ex)
            {
                this.Logger.LogError(ex.Message);
