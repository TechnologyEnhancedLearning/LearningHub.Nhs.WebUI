// <copyright file="UserService.cs" company="NHS England">
...
namespace LearningHub.Nhs.Services
{
    ...
    public class UserService : IUserService
    {
        ...
        public async Task<List<ActiveContentViewModel>> GetActiveContentAsync(int userId)
        {
            ...
            var cacheResponse = await this.cachingService.GetAsync<List<ActiveContentViewModel>>(cacheKey);
        ...
        {
            var activeContent = await this.GetActiveContentAsync(userId);
            ...
            string cacheKey = $"{CacheKeys.ActiveContent}:{userId}";
            await this.cachingService.SetAsync(cacheKey, activeContent);
        ...
        public async Task<LearningHubValidationResult> ReleaseActiveContent(ActiveContentReleaseViewModel activeContentReleaseViewModel)
        {
            ...
            {
                var activeContent = await this.GetActiveContentAsync(activeContentReleaseViewModel.UserId);
        ...
        public async Task<PagedResultSet<UserAdminBasicViewModel>> GetUserAdminBasicPageAsync(int page, int pageSize, string sortColumn = "", string sortDirection = "", string presetFilter = "", string filter = "")
        {
            ...
            result.Items = await this.mapper.ProjectTo<UserAdminBasicViewModel>(items).ToListAsync();
        ...
        {
            var user = await this.userRepository.GetByUsernameAsync(userName, true);
        ...
        public async Task<UserLHBasicViewModel> GetByIdAsync(int id)
        {
            ...
            return this.mapper.Map<UserLHBasicViewModel>(user);
        }

        /// <inheritdoc/>
        public bool IsAdminUser(int userId)
        {
            return this.userRepository.IsAdminUser(userId);
        }
        ...
        public async Task<LearningHubValidationResult> CreateUserAsync(int userId, UserCreateViewModel userCreateViewModel)
        {
            ...
            var retVal = await this.ValidateAsync(user);

            if (retVal.IsValid)
            {
                retVal.CreatedId = await this.userRepository.CreateAsync(userId, user);
            }
